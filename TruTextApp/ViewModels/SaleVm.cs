...
namespace TruTextApp
{
    ...
    class SaleVm : INotifyPropertyChanged /*****/
    {
        private List<SaleCl> recordList;  /*****/
        ...
        private void Add()
        {
            SaleCl newRecord = new SaleCl { SalesID = 0 };  /*****2*/
        ...
        private async void SaveAsync()
        {
            ...
            if (this.ValidateRecord(this.Current))
            {
                ...
                try
                {
                    ...
                    {
                        var response =
                            await client.PostAsync("api/Sales", content);  /*****/
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (newRecord.IsSuccessStatusCode)
                            {
                                ...
                                this.CopyRecord(
                                    JsonConvert.DeserializeObject<SaleCl>(recordData), this.Current);  /*****/
                                this.OnPropertyChanged(nameof(Current));
                                this.IsAdding = false;
                                this.IsBrowsing = true;
                                this.LastError = String.Empty;
                    ...
                    {
                        string path = $"api/Sales/{this.Current.SalesID}";  /*****2*/
        ...
        public async Task GetDataAsync()
        {
            try
            {
                ...
                var response = await this.client.GetAsync("api/Sales");  /*****/
                if (response.IsSuccessStatusCode)
                {
                    var recordData = await response.Content.ReadAsStringAsync();

                    this.recordList = JsonConvert.DeserializeObject<List<SaleCl>>(recordData);  /*****/
