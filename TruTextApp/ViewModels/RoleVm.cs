...
{
    public class RoleVm : INotifyPropertyChanged
    {
        private List<RoleCl> recordList;
        ...
        private void Add()
        {
            RoleCl newRecord = new RoleCl { RoleId = 0 };
        ...
        private async void SaveAsync()
        {
            ...
            if (this.ValidateRecord(this.Current))
            {
                ...
                try
                {
                    ...
                    {
                        var response =
                            await client.PostAsync("api/roles", content);
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (newRecord.IsSuccessStatusCode)
                            {
                                ...
                                this.CopyRecord(
                                    JsonConvert.DeserializeObject<RoleCl>(recordData), this.Current);
                                this.OnPropertyChanged(nameof(Current));
                                this.IsAdding = false;
                                this.IsBrowsing = true;
                                this.LastError = String.Empty;
                    ...
                    {
                        string path = $"api/roles/{this.Current.RoleId}";
        ...
        public async Task GetDataAsync()
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    this.recordList = JsonConvert.DeserializeObject<List<RoleCl>>(recordData);
