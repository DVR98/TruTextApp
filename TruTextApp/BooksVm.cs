...
namespace TruTextApp
{
    ...
    {
        private List<BooksCl> recordList;
        ...
        private void Add()
        {
            BooksCl newRecord = new BooksCl { BookID = 0 };
        ...
        private async void SaveAsync()
        {
            ...
            if (this.ValidateRecord(this.Current))
            {
                ...
                try
                {
                    ...
                    if (this.IsAdding)
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            if (newRecord.IsSuccessStatusCode)
                            {
                                ...
                                this.CopyRecord(
                                    JsonConvert.DeserializeObject<BooksCl>(recordData), this.Current);
                                this.OnPropertyChanged(nameof(Current));
                                this.IsAdding = false;
                                this.IsBrowsing = true;
                                this.LastError = String.Empty;
                    ...
                    {
                        string path = $"api/Books/{this.Current.BookID}";
        ...
        public async Task GetDataAsync()
        {
            try
            {
                ...
                var response = await this.client.GetAsync("api/Books");
                if (response.IsSuccessStatusCode)
                {
                    var recordData = await response.Content.ReadAsStringAsync();

                    this.recordList = JsonConvert.DeserializeObject<List<BooksCl>>(recordData);
